...
func BeginBlock(context sdk.Context, _ abci.RequestBeginBlock, gasTrackingKeeper GasTrackingKeeper, rewardTransferKeeper RewardTransferKeeper, mintParamsKeeper MintParamsKeeper) {
	...
	if totalGasRewardsPerBlock != nil && !totalGasRewardsPerBlock.IsZero() {
		...
		err := rewardTransferKeeper.SendCoinsFromModuleToModule(context, authTypes.FeeCollectorName, GasRewardCollector, totalGasRewardToBeCollected)
		if err != nil {
			panic(err)
		}
	...
	if totalInflationRewardsPerBlock != nil && !totalInflationRewardsPerBlock.IsZero() {
		...
		err := rewardTransferKeeper.SendCoinsFromModuleToModule(context, authTypes.FeeCollectorName, InflationRewardAccumulator, totalInflationRewardToBeCollected)
		if err != nil {
			panic(err)
		}
...
func recordInflationRewards(context sdk.Context, gasTrackingKeeper GasTrackingKeeper, contractAddresses []string, inflationRewardByContractAddress map[string]sdk.DecCoins) {
...
func distributeGasRewards(context sdk.Context, rewardAddresses []string, rewardsByAddress map[string]sdk.DecCoins, gasTrackingKeeper GasTrackingKeeper, rewardTransferKeeper RewardTransferKeeper) {
	for _, rewardAddressStr := range rewardAddresses {
		...
		err = rewardTransferKeeper.SendCoinsFromModuleToAccount(context, GasRewardCollector, rewardAddress, rewardsToBePayed)
		if err != nil {
			panic(err)
		}
...
func getContractRewards(context sdk.Context, blockGasTracking gstTypes.BlockGasTracking, gasTrackingKeeper GasTrackingKeeper, contractTotalInflationRewards sdk.DecCoin) contractRewardData {
	...
	for _, txTrackingInfo := range blockGasTracking.TxTrackingInfos {
		...
		for _, contractTrackingInfo := range txTrackingInfo.ContractTrackingInfos {
			...
			gasConsumedInContract := sdk.NewDecFromBigInt(ConvertUint64ToBigInt(contractTrackingInfo.OriginalSdkGas + contractTrackingInfo.OriginalVmGas))
