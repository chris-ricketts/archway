...
func (chain *TestChain) GetMerklePrefix() commitmentTypes.MerklePrefix {
	return commitmentTypes.NewMerklePrefix(
		chain.app.GetIBCKeeper().ConnectionKeeper.GetCommitmentPrefix().Bytes(),
	)
}
...
func (chain *TestChain) SendIBCPacket(packet exported.PacketI) {
	...
	cap, ok := chain.app.GetScopedIBCKeeper().GetCapability(chain.GetContext(), capPath)
	require.True(t, ok)

	require.NoError(t, chain.app.GetIBCKeeper().ChannelKeeper.SendPacket(chain.GetContext(), cap, packet))
...
func (chain *TestChain) GetIBCPacketCommitment(packet channelTypes.Packet) []byte {
	return chain.app.GetIBCKeeper().ChannelKeeper.GetPacketCommitment(
		chain.GetContext(),
...
func (chain *TestChain) GetNextIBCPacketSequence(portID, channelID string) uint64 {
	...
	seq, ok := chain.app.GetIBCKeeper().ChannelKeeper.GetNextSequenceRecv(chain.GetContext(), portID, channelID)
	require.True(t, ok)
