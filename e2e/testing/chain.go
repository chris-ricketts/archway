...
func (chain *TestChain) GetBalance(accAddr sdk.AccAddress) sdk.Coins {
	return chain.app.GetBankKeeper().GetAllBalances(chain.GetContext(), accAddr)
}

// GetContext returns a context for the current block.
func (chain *TestChain) GetContext() sdk.Context {
	return chain.app.GetBaseApp().NewContext(false, chain.curHeader)
}
...
func (chain *TestChain) GetUnbondingTime() time.Duration {
	return chain.app.GetStakingKeeper().UnbondingTime(chain.GetContext())
}
...
func (chain *TestChain) SendMsgs(senderAcc Account, expPass bool, msgs ...sdk.Msg) (sdk.GasInfo, *sdk.Result, error) {
	...
	senderAccI := chain.app.GetAccountKeeper().GetAccount(chain.GetContext(), senderAcc.Address)
